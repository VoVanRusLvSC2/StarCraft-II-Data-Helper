                                     ...
                                     const QString &targetRootId, const QSet<int> &includedNodeIndices) const
{
    ...
    for (const UnitFamilyObject &object : family.objects)
    {
        ...
        if (!includedNodeIndices.isEmpty() && !includedNodeIndices.contains(object.nodeIndex))
            continue;
        ...
        if (node.elementName.startsWith(QStringLiteral("CModel"), Qt::CaseInsensitive) && scopesById.value(node.id.toCaseFolded()).contains(QStringLiteral("cactor")))
        {
            UnitFamilyObject manual = object;
            manual.confidence = QStringLiteral("Low");
        ...
        else if (object.role == UnitFamilyRole::Actor && node.elementName.startsWith(QStringLiteral("CActorUnit"), Qt::CaseInsensitive))
        {
    ...
    for (RenamePlanItem &item : result.items)
    {
        ...
        {
            item.blocked = true;
            item.conflict = QStringLiteral("Target ID already exists");
        }
        if (item.blocked)
            result.conflicts << QStringLiteral("%1 -> %2: %3").arg(item.oldId, item.newId, item.conflict);
    }
    if (!result.manualReview.isEmpty())
        result.warnings << QStringLiteral("Unsupported ambiguous objects require manual review and are excluded from renaming.");
